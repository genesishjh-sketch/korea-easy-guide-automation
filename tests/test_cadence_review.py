...
class CadenceReviewTests(unittest.TestCase):
    ...
    def test_reddit_health_attention_does_not_block_when_search_signals_exist(self) -> None:
        review = review_cadence(
            today=date(2026, 7, 22),
            published_posts=25,
            indexed_pages_estimate=25,
        ...
        self.assertEqual(review.action, "하루 2개 전환 검토 가능")
        self.assertFalse(review.reddit_health_blocks_cadence_increase)
        self.assertIn("Google site:reddit.com", " ".join(review.reasons))
    ...
    def test_initial_period_still_mentions_reddit_health_blocker(self) -> None:
        ...
        self.assertIn("fallback 질문", joined_reasons)
    ...
    def test_google_site_search_signals_allow_three_post_review_without_oauth(self) -> None:
        review = review_cadence(
        ...
        self.assertEqual(review.current_recommendation, "review_3_posts")
        self.assertEqual(review.action, "하루 3개 전환 검토 가능")
        self.assertIn("Google site:reddit.com", " ".join(review.reasons))
        self.assertEqual(review.reddit_google_site_search_signal_count, 8)

    def test_cadence_alert_omits_reddit_setup_links_when_search_signals_exist(self) -> None:
        review = review_cadence(
            today=date(2026, 7, 22),
            published_posts=25,
            indexed_pages_estimate=25,
